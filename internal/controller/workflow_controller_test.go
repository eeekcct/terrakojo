...
var _ = Describe("Workflow Controller", func() {
	...
	When("reconciling Workflow resources (unit paths)", func() {
		...
		It("updates workflow when legacy job exists", func() {
			ctx := context.Background()
			testScheme := newWorkflowTestScheme()
			...
			legacyJobName := workflow.Name + "-job"
			job := &batchv1.Job{
				ObjectMeta: metav1.ObjectMeta{
					Name:      legacyJobName,
					Namespace: workflow.Namespace,
			...
			missing := &batchv1.Job{}
			err = fakeClient.Get(ctx, client.ObjectKey{Name: workflow.Name, Namespace: workflow.Namespace}, missing)
			Expect(apierrors.IsNotFound(err)).To(BeTrue())
			Expect(fakeClient.Get(ctx, client.ObjectKey{Name: legacyJobName, Namespace: workflow.Namespace}, &batchv1.Job{})).To(Succeed())
		})
